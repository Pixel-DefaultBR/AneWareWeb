...
$(document).ready(function () {
    ...
    const rootUri = 'https://localhost:7084/' // If necessary change this! (In production enviroment)

    if (window.location.href !== rootUri) {
...
function getSeverity() {
    ...
    $(severityStatus).each((index, element) => {
        ...
        if ($(severityStatus[index]).text() === 'critical') {
            statusIcon.src = '../img/critical-status.png';
            $(severityLogo[index]).append(statusIcon);
        } else if ($(severityStatus[index]).text() == 'high') {
            statusIcon.src = '../img/high-status.png';
            $(severityLogo[index]).append(statusIcon);
        } else if ($(severityStatus[index]).text() == 'medium') {
            statusIcon.src = '../img/medium-status.png';
            $(severityLogo[index]).append(statusIcon);
        } else if ($(severityStatus[index]).text() == 'low') {
            statusIcon.src = '../img/low-status.png';
            $(severityLogo[index]).append(statusIcon);
        } else if ($(severityStatus[index]).text() == 'informative') {
